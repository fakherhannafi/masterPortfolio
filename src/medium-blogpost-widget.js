...
class MediumBlogpost extends HTMLElement {
  ...
  renderUser(data) {
    ...
    ).innerHTML = `<img src="${data.feed.image}" alt="${this.name}"/>`;
    this._shadowRoot.querySelector(
      ".medium-blogpost-author-info"
    ).innerHTML = `<h3>${data.items[0].author}</h3><p>@${this.username}</p><a href="https://medium.com/@${this.username}" target="_blank"><button class="medium-follow-button">Follow</button></a>`;
  }
  ...
  renderArticles(data = []) {
    data.forEach((data) => {
      this._shadowRoot.querySelector(
        ".medium-blogpost-articles"
      ).innerHTML += `<a style="background: linear-gradient(to bottom,rgba(37, 37, 37, 0.349), rgba(27, 27, 27, 0.678)), url('${
        data.thumbnail
      }');background-size: contain;"class="medium-blogpost-single-article" href="${
        data.link
      }" target="_blank"><h3>${data.title}</h3><p>${this.parseDate(
        data.pubDate
      )}</p></a>`;
    });
  }
